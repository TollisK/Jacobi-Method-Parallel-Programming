...
#define DST(XX, YY) u[(YY) * (n + 2) + (XX)]
  ...
  while ((++iterationCount) < maxIterationCount && error > maxAcceptableError) {
    error = 0.0;
    // Coefficients
    for (y = 1; y < size_for_m; y++) {
      for (x = 1; x < size_for_n; x++) {
        fXX = (1.0 - pow((xLeft + (x - 1) * deltaX),2.0));
        fYY = (1.0 - pow((yBottom + (y - 1) * deltaY),2.0));
        f = -alpha * fXX * fYY - 2.0 * fXX - 2.0 * fYY;
        updateVal = ((SRC(x - 1, y) + SRC(x + 1, y)) * cx +
                     (SRC(x, y - 1) + SRC(x, y + 1)) * cy +
                     SRC(x, y) * cc - f) /
                    cc;
        DST(x, y) = SRC(x, y) - relax * updateVal;
        error += pow(updateVal,2.0);
      }
    }
    
    error = sqrt(error) / total_size;
    // Swap the buffers
    tmp = u_old;
    u_old = u;
    u = tmp;
